      ...
      <div class="cart-body">
        ...
        >
          ...
          <li class="cart-list-con5">
            <a class="mins" @click="minusSkuNum(cart)" href="javascript:">-</a>
            <input
              autocomplete="off"
              type="text"
              :value="cart.skuNum"
            ...
            />
            <a @click="addSkuNum(cart)" class="plus" href="javascript:">+</a>
          </li>
...
export default {
  ...
  methods: {
    //获取购物车的数据请求函数
    getData() {
      let result = this.$store.dispatch("getShopCart");
    },
    //修改商品数据->加的操作
    async addSkuNum(cart) {
      ...
      try {
        //修改商品个数成功
        await this.$store.dispatch("getGoodsInfo", params);
        //再次获取最新的购物车的数据
        this.getData();
      } catch (error) {
        alert("修改数量失败");
